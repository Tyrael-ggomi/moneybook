...
DB_PATH = Path(
    os.environ.get('MONEYBOOK_DB', APP_DIR / 'moneybook.db')
).resolve()
...
PORT = int(
    os.environ.get(
        'PORT',
        os.environ.get('MONEYBOOK_PORT', '8765')
    )
)
...
def db():
    """
    Render/Supabase uses PostgreSQL.
...
def insert_and_get_id(conn, sql, params=()):
    ...
    따라서 로컬 SQLite와 Render/Supabase 양쪽에서
    같은 app.py를 사용할 수 있다.
    """
...
def ensure_schema():
    """
    SQLite:
        기존 로컬 DB의 부족한 컬럼/미정 카테고리를 보완.

    PostgreSQL:
        Supabase migration으로 테이블이 생성되었다고 가정.
    """
    ...
    with db() as conn:
        ...
        if 'tx_time' not in tx_cols:
            conn.execute(
                ...
                ADD COLUMN tx_time TEXT NOT NULL DEFAULT ''
                """
            )
        ...
        if 'period_start_day' not in card_cols:
            conn.execute(
                """
                ALTER TABLE cards
                ADD COLUMN period_start_day INTEGER
                """
            )
        ...
        if 'period_end_day' not in card_cols:
            conn.execute(
                """
                ALTER TABLE cards
                ADD COLUMN period_end_day INTEGER
                """
            )
...
def validate_transaction_payload(
    ...
    tx_date = normalize_date(
        data.get('date')
    )

    tx_time = normalize_time(
        data.get('time')
    )
    ...
    if not 0 <= user_percent <= 100:
        raise ValueError(
            '내 비율은 0~100% 사이여야 합니다.'
        )

    if not 1 <= installment_months <= 60:
        raise ValueError(
            '할부 개월은 1~60개월입니다.'
        )
    ...
    if category_id in (
        ...
        category_id = apply_auto_category(
            conn,
            content
        )
    else:
...
def api_update_transaction(
    ...
    with db() as conn:
        ...
        values = validate_transaction_payload(
            conn,
            data
        )
...
def api_check_duplicates(handler):
    ...
    with db() as conn:
        ...
        for row in rows[:500]:
            ...
            internal_similar = (
                simkey in seen_similar
            )

            seen_exact.add(key)
            seen_similar.add(simkey)
        ...
        if method == 'GET':
            ...
            if kind == 'names':
                ...
                json_response(
                    handler,
                    {
                        'user_name': settings.get(
                            'user_name',
                            '나'
                        ),
                        'wife_name': settings.get(
                            'wife_name',
                            '배우자'
                        )
                    }
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == '/api/bootstrap':
                ...
                r'/api/settings/(cards|categories|rules|names)',
                path
            ):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == '/api/transactions':
                ...
                r'/api/settings/(cards|categories|rules)',
                path
            ):
                ...
                r'/api/settings/(cards|categories|rules)/\d+',
                path
            ):
