...
function requireEl(id){const el=$(id);if(!el)throw new Error(`설정 화면이 최신 버전으로 로드되지 않았습니다. 페이지를 새로고침해 주세요. (누락: ${id})`);return el}let boot=null,editingId=null,lastTransactions=[],importDuplicateFlags=[];
...
function render(d){$('count').textContent=`총 ${d.total}건`;if(!d.transactions.length){$('list').innerHTML='<div class="empty">조건에 맞는 지출 내역이 없습니다.</div>';return} $('list').innerHTML=d.transactions.map(t=>`<button class="item itemBtn" data-id="${t.id}" type="button"><div class="row"><div class="itemMain"><div class="date">${t.tx_date}${t.tx_time?` · ${esc(t.tx_time)}`:''}</div><div class="content">${esc(t.content||'(내용 없음)')}</div><div class="meta">${esc(t.card_name)} · ${esc(t.category_name)} · ${t.user_percent}:${t.wife_percent}${t.installment_months>1?' · '+t.installment_months+'개월':' · 일시불'}</div></div><div class="amount ${t.amount<0?'negative':''}">${money(t.amount)}</div></div></button>`).join('');document.querySelectorAll('.itemBtn').forEach(x=>x.onclick=()=>openEdit(+x.dataset.id))}
...
async function loadSettings(){
  try{
    ...
    const renderOrderList=(items,kind)=>items.map((x,i)=>{
      const up=i>0?settingButton('↑','orderBtn',`onclick="moveSetting('${kind}',${x.id},-1)"`):'';
      const down=i<items.length-1?settingButton('↓','orderBtn',`onclick="moveSetting('${kind}',${x.id},1)"`):'';
      ...
      return `<div class="settingsItem ${x.active?'':'inactive'}"><div class="settingsOrder">${up}${down}</div><div class="settingsMain"><div class="settingsName">${esc(x.name)}</div><div class="settingsSub">${sub}${extra}</div></div>${settingButton('수정','settingsActionBtn',`onclick="settingsEditClick('${kind}',${x.id})"`)}${settingButton(x.active?'중지':'사용','danger settingsActionBtn','data-settings-toggle="'+kind+'" data-id="'+x.id+'"')}</div>`;
    ...
    $('rulesSettingsList').innerHTML=rules.items.map(x=>`<div class="settingsItem ${x.active?'':'inactive'}"><div class="settingsMain"><div class="settingsName">${esc(x.keyword)}</div><div class="settingsSub">→ ${esc(x.category_name)} · ${x.active?'사용 중':'사용 중지'}</div></div>${settingButton('수정','settingsActionBtn',`onclick="settingsEditClick('rules',${x.id})"`)}${settingButton(x.active?'중지':'사용','danger settingsActionBtn','data-settings-toggle="rules" data-id="'+x.id+'"')}</div>`).join('')||'<div class="empty">자동분류 규칙이 없습니다.</div>';
...
async function moveSetting(kind,id,direction){
  const arr=kind==='cards'?cardsCache:catsCache;
...
// v0.6.4 — Excel / Google Sheets import
let importRows=[];
