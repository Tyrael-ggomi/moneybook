...
function bindSettingDrag(){document.querySelectorAll('#settingsView .settingsItem[data-setting-kind]').forEach(el=>{el.draggable=true;el.addEventListener('dragstart',e=>{e.dataTransfer.setData('text/plain',el.dataset.id);el.classList.add('dragging')});el.addEventListener('dragend',()=>el.classList.remove('dragging'));el.addEventListener('dragover',e=>{e.preventDefault();el.classList.add('dragOver')});el.addEventListener('dragleave',()=>el.classList.remove('dragOver'));el.addEventListener('drop',async e=>{e.preventDefault();el.classList.remove('dragOver');const from=el.parentElement.querySelector('.dragging');if(!from||from===el)return;const items=[...el.parentElement.querySelectorAll('.settingsItem[data-setting-kind]')];const ids=items.map(x=>Number(x.dataset.id));const a=ids.indexOf(Number(from.dataset.id)),b=ids.indexOf(Number(el.dataset.id));ids.splice(a,1);ids.splice(b,0,Number(from.dataset.id));try{await apiSettings('order',null,'POST',{kind:el.dataset.settingKind,ids});await loadSettings();await refreshAfterSettings()}catch(err){msg(err.message)}})})}
